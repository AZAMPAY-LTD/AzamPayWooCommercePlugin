 ...
 *
 * @class       WC_Gateway_Azampay
 * @extends     WC_Payment_Gateway
 * @version     1.0.0
 * @package     WooCommerce\Classes\Payment
 */

define('WC_AZAMPAY_VERSION', '1.0');
...
class WC_Gateway_Azampay extends WC_Payment_Gateway
{
  ...
  {
    $this->id                 = 'azampay';
    $this->icon               = apply_filters('woocommerce_azampay_icon', plugins_url('/assets/images/logo.png', __FILE__));
  ...
  public function is_valid_for_use()
  {
    ...
    if (!in_array(get_woocommerce_currency(), apply_filters('woocommerce_azampay_supported_currencies', $supported_currencies))) {
      $this->msg = sprintf(__('AzamPay does not support your store currency. Kindly set it to Tanzanian Shillings (TZS) <a href="%s">here</a>', 'azampay-woo'), admin_url('admin.php?page=wc-settings&tab=general'));
      return false;
    }
  ...
  public function admin_notices()
  {
    ...
    if ($this->test_mode && !($this->test_client_id && $this->test_client_secret && $this->test_app_name && $this->test_callback_token)) {
      echo '<div class="error"><p>' . sprintf(__('Please enter your AzamPay merchant details for test <a href="%s">here</a> to be able to use the AzamPay WooCommerce plugin.', 'azampay-woo'), admin_url('admin.php?page=wc-settings&tab=checkout&section=azampay')) . '</p></div>';
      return;
    } else if (!($this->prod_client_id && $this->prod_client_secret && $this->prod_app_name && $this->prod_callback_token)) {
      echo '<div class="error"><p>' . sprintf(__('Please enter your AzamPay merchant details for production <a href="%s">here</a> to be able to use the AzamPay WooCommerce plugin.', 'azampay-woo'), admin_url('admin.php?page=wc-settings&tab=checkout&section=azampay')) . '</p></div>';
      return;
    }
  }
  ...
  public function is_available()
  {
    ...
    if ($this->enabled) {
      ...
      if (!($this->app_name && $this->client_id && $this->client_secret && $this->callback_token)) {
        return false;
      }
      return true;
    }
    ...
    if ($this->is_valid_for_use()) {
        ...
        <tr valign="top" style="display:none;">
          ...
          <td id="woocommerce_<?php echo esc_attr($this->id) ?>_allowed_partners" class="forminp">
            <fieldset style="display:flex; gap:15px;">
              ...
              <?php echo wp_kses($partnersHTML, array(
                'label' => array(
                  'for' => array()
                ),
                'input' => array(
      ...
      ?>
        <div class="inline error">
          <p><strong><?php _e('AzamPay Payment Gateway Disabled', 'azampay-woo'); ?></strong>: <?php echo $this->msg; ?></p>
        </div>
  ...
  public function admin_scripts()
  {
    ...
    wp_enqueue_script('wc_azampay_admin', plugins_url('assets/js/azampay-admin.js', __FILE__), array(), WC_AZAMPAY_VERSION, true);
  ...
  private function generate_token()
  {
    ...
    if (is_wp_error($token_request) || $token_response_code !== 200) {
      ...
      if ($token_response_code === 423) {
        ...
        $result['message'] = 'Provided detail is not valid for this app or secret key has been expired.';
      } else if ($token_response_code === 500) {
  ...
  private function get_partners()
  {
    ...
    } else if (!is_array($partners_response) && property_exists($partners_response, 'status') && $partners_response->status === 'Error') {
  ...
  public function payment_fields()
  {
    ...
    wp_enqueue_style('styles', plugins_url('/assets/css/azampay-styles.css', __FILE__), array(), false);

    if ($this->description) {

      if ($this->test_mode) {
        $this->description .= '<p class="form-row form-row-wide" style="margin-top:5px">TEST MODE ENABLED. In Sandbox, you can use the AzamPesa numbers listed below to proceed with tests for the different scenarios.
        0555 123 456
        </p>';
    ...
    if (!$this->token_result['success'] || !$this->partners_result['success']) {
      ...
      echo '<script type="text/javascript">
              ...
              jQuery("#payment_method_' . esc_js($this->id) . '").prop("disabled", true);
            </script>';
    }
    ...
    if (!$this->token_result['success']) {
      ...
      $admin_message = '<a href="' . admin_url('admin.php?page=wc-settings&tab=checkout&section=' . esc_attr($this->id)) . '" target="_blank">Click here to configure the plugin</a>.';
    ...
    } else if (!$this->partners_result['success']) {
          ...
          $azampesa_field = '<div class="form-row form-row-wide azampesa-label-container">
                              <label class="azampesa-container">
                                ...
                                <div class="azampesa-right-block">
                                  ...
                                  <img class="azampesa-img" src=' . plugins_url('/assets/images/azampesa-logo.svg', __FILE__) . ' alt=' . esc_attr($partner_value) . '>
                                </div>
                              </label>
                            </div>';
        } else {
          $logo_path = '/assets/images/' . esc_attr($partner_name).'-logo.svg';
          $other_fields .= '<label>
      ...
      echo '<script type="text/javascript">
              jQuery("#payment_method_' . esc_js($this->id) . '").prop("disabled", false);
  ...
  private function azampay_payment_processing($order)
  {
    ...
    if (!$this->token_result['success']) {
      ...
      if (is_wp_error($checkout_request) || $checkout_response_code !== 200) {
        ...
        return false;
      } else if (!$checkout_response_body->success) {
  ...
  public function process_webhooks()
  {
    ...
    foreach ($required_fields as $field) {
      ...
      if (!property_exists($data, $field)) {
        ...
        http_response_code(400);
        echo ($field . ' must be specified in payload.');
        exit;
      }
    }
    ...
    if (is_null($order)) {
      ...
      http_response_code(400);
      echo ('Order with given order id does not exist.');
      exit;
    }
    ...
    if (in_array($order_status, array('processing', 'completed', 'on-hold'))) {
      echo ('Order has already been processed.');
      exit;
    }
    ...
    if (is_null($transaction_status)) {
      ...
      http_response_code(400);
      echo ('Transaction status not specified.');
      exit;
    }
