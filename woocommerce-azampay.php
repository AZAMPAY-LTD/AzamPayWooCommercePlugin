 ...
 */
define( 'WC_AZAMPAY_VERSION', '1.1.6' ); // WRCS: DEFINED_VERSION.
...
function woo_azampay_testmode_notice() {
  ...
  $azampay_settings = get_option('woocommerce_' . AzamPay_Gateway::ID . '_settings');
...
function woocommerce_azampay() {
    ...
    class Woo_AzamPay {
      ...
      public function plugin_row_meta( $links ) {
        ...
        $row_meta = array(
          'tnc' => '<a href="' . esc_url($tnc_url) . '" aria-label="' . esc_attr__('View AzamPay terms and conditions', 'azampay') . '">' . esc_html__('Terms and Conditions', 'azampay') . '</a>',
          'pp' => '<a href="' . esc_url($pp_url) . '" aria-label="' . esc_attr__('View AzamPay privacy policy', 'azampay') . '">' . esc_html__('Privacy Policy', 'azampay') . '</a>',
        );
      ...
      public function add_currencies_to_store( $currencies) {
        $currencies['TZS'] = __('Tanzanian Shillings', 'azampay');
        return $currencies;
      }
       ...
       * @version 1.1.5
			...
			public function get_main_azampay_gateway() {
				...
				$this->azampay_gateway = new AzamPay_Gateway();
  ...
  if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
    ...
    add_action(
      ...
      function( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
        if ( ! class_exists( 'AzamPay_Gateway' ) ) {
					return;
				}
				...
				$container->register(
					AzamPay_Blocks_Support::class,
					new AzamPay_Blocks_Support()
				);

				$payment_method_registry->register(
					$container->get( AzamPay_Blocks_Support::class )
				);
      }
    );
  }
